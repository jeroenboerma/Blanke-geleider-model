...
Mc, Cpc, rho, rho0, alpha, epsilon = Dyn_Properties.select_material(Material, Shape, Painted, Location)
Dh, ID, w, B, L, A, S, V, Ro, Sr = Dyn_Properties.component_parameters(Shape, Core, rho0, epsilon)
M, Beta, Cpa, mu, labda, v = Dyn_Properties.ambient_parameters()
g, sigma = Dyn_Properties.PhyCo()
phis, r = Dyn_Properties.Wheather(Material)
...
def Qgen(I,T):
    ...
    Rac, Ys, Xs = Dyn_Properties.Rac(Rdc, Shape, Core, Dh, ID, rho0, L, alpha, T, w)
    Qgen = I**2*Rac
...
def Qconv(T, Te, Shape, Location):
    if Location == "Inside":
        ...
        if Shape == "Cylindrical":
            Tf = (T+Te)/2 - 273.15                                      #Film temperature
            Pr = 0.715 - 2.5e-4*Tf                                      #Prandtl number
            Re = v*Dh*(1.644e9*(Te)**(-1.78))                           #Reynolds number according to IEC 60943 note
            ...
            Re = v*Dh*(1.644e9*(Te)**(-1.78))                          #Reynolds number according to IEC 60943 note
            NuL = 0.65*Re**(0.2) + 0.23*Re**(0.61)                     #Nusselt number IEC 60943
            h = NuL*labda/Dh
            Qconv = h*A*(T-Te)      #Natural convection
            
            Ra = 0      #Keep zero as not applicable
            NuD = 0     #Keep zero as not applicable
            ...
            Ra = 0
            NuL = 0
            NuD = 0.65*Re**0.2 + 0.23*Re**0.61  #IEC 60943
            h = NuD*labdaf/Dh
            Qconv = np.pi*labdaf*(T-Te)*NuD*L   #ELT 144_3 
